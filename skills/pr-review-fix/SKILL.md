...
Uses the same definitions as `clean-work-cycle`:
...
2. Extract the worktree path from the PR body (clean-work-cycle puts `Worktree: <path>` in PR descriptions)
...
1. **`mergesync`** — merge the PR now, sync (only offer when invoked standalone, not from within clean-work-cycle)
